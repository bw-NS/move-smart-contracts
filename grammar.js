...
module.exports = grammar({
  ...
  conflicts: $ => [
    ...
    [$.block, $._expression]
    // [$.shit_literal, $.address_literal],
    // [$.let_statement, $._variable_identifier],
  ],
  ...
  rules: {
    ...
    _struct_signature: $ => seq(
      ...
      optional(field('attribute', $.struct_attributes))
    ),
    struct_attributes: $ => seq(
      'has',
      sepBy(',', choice('key', 'store', 'drop', 'copy')),
    ),
    ...
    module_access: $ => 
      ...
      seq(
        ...
        '::',
        field('module_idf_member', $.identifier)
      ),
      seq(
        $._module_identity,
        optional(seq('::',
        field('module_idt_member', $.identifier)))
      ),
    ...
    type_arguments: $ => 
      seq("<", choice($._type, seq("phantom", $._type),),  ">"),
    ...
    type_parameters: $ => seq('<', sepBy1(',', choice($.type_parameter, seq('phantom', $.type_parameter))), '>'),
    type_parameter: $ => seq(
    ...
    block: $ => seq(
      ...
      repeat($.use_decl),
      repeat($._block_item),
      // optional(choice($.let_statement,$._expression)),
      '}'
    ),
    _block_item: $ => seq(
      choice(
    ...
    let_statement: $ => seq(
      // choice(field('binds', $.bind_list),$.identifier),
      // $.identifier,
      // optional(seq(':', field('type', $._type))),
      // optional(seq('=', field('exp', $._expression)))
      'let',
      $.assign_expression
      // field('var_decl', $.identifier),
      // optional(seq('=',
      // field('def', $._expression))),
      // ';'
    ),
    ...
    address_literal: $ => /0x[a-fA-F0-9]+|[a-zA-Z]+/,
